...
//
// parasites (blue) drain energy from your cells (red).
...
// which quadrant of the screen your cursor is in determines wind direction; influences cell movement.
...
void draw() {
  ...
  if (introRun == true) {
    ...
    if (introOne==true) {
      ...
      textSize(48);
      textAlign(CENTER);
      text("will graham-simpkins", 400, 256);
      fill(255, introFade);
      text("presents", 400, 312);
      introFade++;
      introFade++;
      if (introFade >= 255) {
        introOne = false;
    ...
    } else if (introThree == true) {
      if (introFade < 255) {
        fill(255, introFade);
        text("a game by", 400, 256);
        introFade++;
        introFade++;
        if (introFade >= 255) {
          introThree = false;
    ...
    } else if (introFour == true) {
      fill(255, introFade);
      text("a game by", 400, 256);
      fill(255, introFade2);
      text("will graham-simpkins", 400, 312);
      introFade2++;
      introFade2++;
      if (introFade2 >= 255) {
        introFour = false;
    ...
    } else if (introSix == false) {
      image(mainmenu, 0, 0);
      fill(introFade, 0, 0, (255-introFade2));
      ...
      if (introFade2 >= 255) {
        ...
        coda.stop();
        menu_bgm.play();
        menuTime = millis() + 38916;
      }
    }
    println(millis());
    //if (millis() > introTime+256) {
    //  introRun = false;
    //  menu = true;
    //  coda.stop();
    //  menu_bgm.loop();
  ...
  if (menu == true) {
    ...
    // resets the game is song ends on main menu
    if (millis() > menuTime) {

      // song plays again if in "how to play" screen
      if (howToPlay) {
        menu_bgm.play();
        menuTime = millis() + 38916;
        return;
  ...
  if (run == true) {
    ...
    if (severCount == 8) {
      if (boss.energy <= 0) {
        ...
        carlos.stop();             
        if (creditRollY == 600) {
          creditsTime = millis() + 84488;
          ending.play();
        }
        if (millis() > creditsTime) {
          setup();
        }
        image(intro3, 0, 0);
...
void mousePressed() {
  ...
  // allows the intro sequence to be skipped by clicking (you're welcome)
  if ((introOne)||(introTwo)||(introThree)||(introFour)||(introFive)) {
    introOne = false;
    introTwo = false;
    introThree = false;
    introFour = false;
